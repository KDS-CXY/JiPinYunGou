package com.baizhanshopping.shopping_seckill_service.service;
...
import com.baizhanshopping.shopping_seckill_service.mapper.SeckillGoodsMapper;
import com.baizhanshopping.shopping_seckill_service.redis.RedissonLock;
...
public class SeckillServiceImpl implements SeckillService {
    ...
    @Scheduled(cron = "0 * * * * *")
    public void refershRedis(){
        // 将redis中秒杀商品的库存数据同步到mysql
        List<SeckillGoods> seckillGoodsListOld = redisTemplate.boundHashOps("seckillGoods").values();//如果redis中没有数据返回的是空集合而不是null
    ...
    public Page<SeckillGoods> findPageByRedis(int page, int size) {
        ...
        List<SeckillGoods> seckillGoodsPage=seckillGoods.subList(start,end);
        Page<SeckillGoods> selectPage = new Page<>();
        selectPage.setTotal(seckillGoods.size());
        selectPage.setCurrent(page);
        selectPage.setSize(size);
        selectPage.setRecords(seckillGoodsPage);
        return selectPage;
    }
    ...
    public Orders createOrder(Orders orders) {
        String key = orders.getCartGoods().get(0).getOrderId().toString();
        if(redissonLock.lock(key,10000L)){
            try {
                // 1.生成订单对象
